...
class MSK(nn.Module):
    def __init__(self, device="cuda", l_layer=6):
        ...
        self.gpt2 = nn.DataParallel(self.gpt2, device_ids=gpus).cuda()
    ...
    def custom_forward(self,
        ...
        output_attentions = output_attentions if output_attentions is not None else self.gpt2.module.config.output_attentions
        output_hidden_states = output_hidden_states if output_hidden_states is not None else self.gpt2.module.config.output_hidden_states
        use_cache = use_cache if use_cache is not None else self.gpt2.module.config.use_cache
        return_dict = return_dict if return_dict is not None else self.gpt2.module.config.use_return_dict
        ...
        if past_key_values is None:
            ...
            past_key_values = tuple([None] * len(self.gpt2.module.h))
        else:
        ...
        inputs_embeds = self.gpt2.module.wte(inputs_embeds)
        position_embeds = self.gpt2.module.wpe(position_ids)
        ...
        for i, (block, layer_past) in enumerate(zip(self.gpt2.module.h, past_key_values)):
        ...
        hidden_states = self.gpt2.module.ln_f(hidden_states)
    ...
    def forward(self, x, calibrated_mask):
        ...
        num_heads =  self.gpt2.module.config.n_head
...
class GenPromptEmb(nn.Module):
    ...
    def _generate_prompt(self, template_type):
        templates = {
            'GT_HD': "From <t1> to <t2>, the values were <value1, ..., valuen> every month. The value for Y label is <gt1, ..., gtn>",
            'HD':    "From <t1> to <t2>, the values were <value1, ..., valuen> every month. Forecast the value for Y label",
        }
    ...
    def _prepare_prompt(self, GT_HD, HD, x, y, time_ref, i, j):
        values = x[i, :, j].flatten().tolist()
        values_str = ", ".join([str(int(value)) for value in values])

        gt_values = y[i, :].flatten().tolist()
    ...
    def generate_embeddings(self, x, y, time_ref):
        ...
        for i in range(len(x)):
            for j in range(x.shape[2]):
                GT_HD_token, HD_token, gt_token_types, hd_token_types = self._prepare_prompt(GT_HD, HD, x, y, time_ref, i, j)
